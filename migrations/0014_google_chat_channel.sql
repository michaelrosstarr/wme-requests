-- Adds "google_chat" as a notification channel platform — posts to a Google Chat space's
-- incoming webhook, same shape as Slack/Discord (reuses the existing webhook_url column).
-- SQLite can't ALTER a CHECK constraint in place, so the platform check is widened by
-- recreating the table, same as 0005_channel_email.sql / 0008_google_sheets_channel.sql.
...
CREATE TABLE notification_channels_new (
  ...
  label                TEXT    NOT NULL,
  platform             TEXT    NOT NULL CHECK(platform IN ('slack','discord','telegram','email','webhook','google_sheets','google_chat')),
