-- Adds "accept_pur" and "decline_pur" as request types (Place Update Request accept/decline,
-- targeting a WME place/venue instead of a segment) and as notification channel / push
-- subscription event types, so a channel or subscription can be scoped to fire only for one of
-- them. SQLite can't ALTER a CHECK constraint in place, so all three tables carrying a type/
-- event_type CHECK are recreated, same recipe as 0005/0008/0014/0016/0017.
...
CREATE TABLE requests_new (
  ...
  type         TEXT    NOT NULL CHECK(type IN ('downlock','imagery','accept_pur','decline_pur')),
  permalink    TEXT    NOT NULL,
  lock_level   INTEGER,     -- inferred from the selected WME segment/place; downlock and PUR only
  editor_rank  INTEGER,
...
CREATE TABLE notification_channels_new (
  ...
  event_type             TEXT    NOT NULL CHECK(event_type IN ('global','downlock','imagery','accept_pur','decline_pur')),
  webhook_url            TEXT,
  bot_token              TEXT,
  chat_id                TEXT,
  custom_prefix          TEXT,
...
CREATE TABLE push_subscriptions_new (
  ...
  country_id INTEGER NOT NULL REFERENCES countries(id) ON DELETE CASCADE,
  region_id  INTEGER REFERENCES regions(id) ON DELETE CASCADE,
  event_type TEXT    NOT NULL DEFAULT 'global' CHECK(event_type IN ('global', 'downlock', 'imagery', 'accept_pur', 'decline_pur')),
