import type {
  ...
  MapComment,
  SegmentAddress,
  Country as WmeCountry,
  State as WmeRegion,
  SelectionWithLocalizedTypeName,
} from 'wme-sdk-typings';

// ── Domain types ──────────────────────────────────────────────────────────────
type RequestType = 'downlock' | 'uplock' | 'imagery' | 'accept_pur' | 'decline_pur';
type EntityKind = 'segment' | 'venue' | 'mapComment';
type StatusKind = 'ok' | 'error' | 'info';
type FabStyle = 'full' | 'compact' | 'icon';
type StatusFn = (msg: string, type: StatusKind) => void;
...
type SelectedEntity =
  | { kind: 'segment'; items: Segment[] }
  | { kind: 'venue'; items: Venue[] }
  | { kind: 'mapComment'; items: MapComment[] };

type NormalizedSelection =
  | { kind: 'segment'; ids: number[] }
  | { kind: 'venue'; ids: string[] }
  | { kind: 'mapComment'; ids: string[] };
...
const pageWindow = (typeof unsafeWindow !== 'undefined' ? unsafeWindow : window) as Window & typeof globalThis;
...
let getSelectionFn: (() => SelectionWithLocalizedTypeName | null) | null = null;
let getSegmentByIdFn: ((args: { segmentId: number }) => Segment | null) | null = null;
let getMapCommentByIdFn: ((args: { mapCommentId: string }) => MapComment | null) | null = null;
let getVenueByIdFn: ((args: { venueId: string }) => Venue | null) | null = null;
let getSegmentAddressFn: ((args: { segmentId: number }) => SegmentAddress | null) | null = null;
let getTopCountryFn: (() => WmeCountry | null) | null = null;
let getTopStateFn: (() => WmeRegion | null) | null = null;

const LOCK_GATED_TYPES: RequestType[] = ['downlock', 'uplock', 'accept_pur', 'decline_pur'];
...
const TYPE_ENTITY_KINDS: Record<RequestType, EntityKind[]> = {
...
function resolveSdkMethods(): void {
  if (!sdk) return;
...
function injectAsNativeTab(tabsUl: Element, tabContent: Element): void {
...
function injectAsFloatingPanel(): void {
...
function createFloatingActions(): void {
...
function quickActionLabel(type: RequestType): string {
...
function refreshQuickActionLabels(): void {
  document.querySelectorAll<HTMLElement>('[data-wmereq-qa-type]').forEach((btn) => {
    btn.textContent = quickActionLabel(btn.getAttribute('data-wmereq-qa-type') as RequestType);
  });
}
...
function loadFabVisible(): FabVisibility {
  ...
  try {
    ...
    log('loadFabVisible: failed to parse saved visibility: ' + (e as Error).message);
    return defaults;
  }
}
...
function makeDraggable(container: HTMLElement, handle: HTMLElement, storageKey: string): void {
  const saved = GM_getValue<string | null>(storageKey, null);
  if (saved) {
    try {
      const pos = JSON.parse(saved) as { top: number; left: number };
      container.style.top = `${pos.top}px`;
      container.style.left = `${pos.left}px`;
      container.style.right = 'auto';
    } catch (e) {
      log('makeDraggable: failed to parse saved position: ' + (e as Error).message);
    }
  }
...
function normalizeSelection(raw: unknown): NormalizedSelection | null {
  if (!raw || typeof raw !== 'object') return null;
  const r = raw as Record<string, unknown>;
  if (Array.isArray(r.ids)) {
    if (r.objectType === 'mapComment') return { kind: 'mapComment', ids: r.ids as string[] };
    if (r.objectType === 'venue') return { kind: 'venue', ids: r.ids as string[] };
    if (!r.objectType || r.objectType === 'segment') return { kind: 'segment', ids: r.ids as number[] };
    return null;
  }
  if (Array.isArray(r.segmentIds)) return { kind: 'segment', ids: r.segmentIds as number[] };
  if (Array.isArray(raw)) {
    const arr = raw as Record<string, unknown>[];
    const segIds = arr
      .filter((f) => (f.objectType || f.type) === 'segment' || f.segmentId != null)
      .map((f) => (f.id ?? f.segmentId) as number);
    if (segIds.length) return { kind: 'segment', ids: segIds };
    const noteIds = arr.filter((f) => (f.objectType || f.type) === 'mapComment').map((f) => f.id as string);
    if (noteIds.length) return { kind: 'mapComment', ids: noteIds };
    const venueIds = arr.filter((f) => (f.objectType || f.type) === 'venue').map((f) => f.id as string);
    if (venueIds.length) return { kind: 'venue', ids: venueIds };
  }
  return null;
}

function getSelectedEntity(): SelectedEntity | null {
...
function getSelectedEntityFromSdk(): SelectedEntity | null {
  ...
  try {
    ...
    if (normalized.kind === 'segment') {
      ...
      const items = normalized.ids.map((segmentId) => getSegmentByIdFn!({ segmentId })).filter((s): s is Segment => s != null);
      return items.length ? { kind: 'segment', items } : null;
    }
    if (normalized.kind === 'mapComment') {
      if (!getMapCommentByIdFn) return null;
      const items = normalized.ids.map((mapCommentId) => getMapCommentByIdFn!({ mapCommentId })).filter((s): s is MapComment => s != null);
      return items.length ? { kind: 'mapComment', items } : null;
    }
    if (normalized.kind === 'venue') {
      if (!getVenueByIdFn) return null;
      const items = normalized.ids.map((venueId) => getVenueByIdFn!({ venueId })).filter((s): s is Venue => s != null);
      return items.length ? { kind: 'venue', items } : null;
    }
    return null;
  } catch (e) {
    log('getSelectedEntity failed: ' + (e as Error).message);
    return null;
  }
}

function getVenueEntityFromUrl(): SelectedEntity | null {
  if (!getVenueByIdFn) return null;
  ...
  try {
    ...
    log('getVenueEntityFromUrl failed: ' + (e as Error).message);
    return null;
  }
}
...
function groupByLockLevel(items: Segment[]): { level: number | null; items: Segment[] }[] {
...
function matchByNameOrCode<T extends { id: number; name: string; code: string }>(
  list: T[],
  obj: WmeCountry | WmeRegion | null | undefined,
): T | null {
  if (!obj) return null;
  ...
  return (
    list.find(
      (item) => (abbr && item.code.toLowerCase() === abbr) || item.name.toLowerCase() === name,
    ) || null
  );
}
...
function getSegmentAddress(seg: Segment | null | undefined): SegmentAddress | null {
  ...
  try {
    ...
    debugLog('getSegmentAddress: threw: ' + (e as Error).message);
    return null;
  }
}
...
function resolveCurrentCountryId(seg: Segment | null | undefined): number | null {
  ...
  try {
    ...
    debugLog('resolveCurrentCountryId: getTopCountry threw: ' + (e as Error).message);
    return null;
  }
}
...
function findUserInfo(): Record<string, unknown> | null {
  if (!sdk?.State) {
  ...
  const state = sdk.State as unknown as Record<string, unknown>;

  if (state.userInfo) {
    if (DEBUG) debugLog(`findUserInfo: sdk.State.userInfo (property) = ${JSON.stringify(state.userInfo)}`);
    return state.userInfo as Record<string, unknown>;
  }
  if (typeof sdk.State.getUserInfo === 'function') {
    ...
    if (result) return result as unknown as Record<string, unknown>;
  }
  if (typeof state.get === 'function') {
    const result = (state.get as (key: string) => unknown)('userInfo');
  ...
  if (sdkRec.User) {
    ...
    const user = sdkRec.User as Record<string, unknown>;
    if (user.userInfo) return user.userInfo as Record<string, unknown>;
    if (typeof user.getUserInfo === 'function') return (user.getUserInfo as () => Record<string, unknown>)();
  }
...
function getCurrentUserInfo(): { userName: string; editorRank: number | null } {
  const info = findUserInfo();
  const userName = (pick(info, ['userName', 'username', 'nickname']) as string | null) || '';
  const rankRaw = pick(info, ['rank', 'editingRank', 'level']) as number | null;
...
const NATIVE_PANEL_TYPES: Partial<Record<EntityKind, RequestType[]>> = {
...
function injectNativeQuickActions(kind: EntityKind | null, attempt?: number): void {
...
function buildPermalink(items: EntityItem[], kind: EntityKind): string {
  try {
    const center = sdk!.Map.getMapCenter();
    const zoom = sdk!.Map.getZoomLevel();
    const ids = items.map((s) => s.id).filter((id) => id != null).join(',');
...
function stripUpdateRequestParams(href: string): string {
  try {
...
const DOWNLOCK_REASON_TOOLTIPS: Record<string, string> = {
...
const PUR_REASON_TOOLTIPS: Record<string, string> = {
...
const PLACE_LOCK_REASON_TOOLTIPS: Record<string, string> = {
...
function getReasonModalConfig(type: RequestType, kind: EntityKind | null): ReasonModalConfig | null {
  if (type === 'downlock') {
    return kind === 'venue'
...
function openReasonModal({ title, reasons, tooltips, requireLevel }: ReasonModalConfig & { requireLevel?: boolean }): Promise<ReasonModalResult> {
  return new Promise((resolve) => {
    ...
    dlg.querySelectorAll<HTMLButtonElement>('.wmereq-chip').forEach((btn) => {
    ...
    dlg.querySelector('#wmereq-reason-confirm')!.addEventListener('click', () => {
      const levelSel = dlg.querySelector<HTMLSelectElement>('#wmereq-reason-level');
      if (requireLevel && levelSel && !levelSel.value) { levelSel.focus(); return; }
      const custom = dlg.querySelector<HTMLTextAreaElement>('#wmereq-reason-custom')!.value.trim();
...
// ── Submit ────────────────────────────────────────────────────────────────────
async function submitRequest(type: RequestType): Promise<void> {
  const countryId = byId<HTMLSelectElement>('wmereq-country')?.value ?? '';
  const regionId = byId<HTMLSelectElement>('wmereq-region')?.value ?? '';
  const lockLevel = byId<HTMLSelectElement>('wmereq-lock')?.value ?? '';
  let notes = byId<HTMLTextAreaElement>('wmereq-notes')?.value.trim() ?? '';
...
async function doSubmit(type: RequestType, { countryId, regionId, lockLevel, notes, status }: DoSubmitOptions): Promise<void> {
  ...
  const groups: RequestGroup[] = LOCK_GATED_TYPES.includes(type) && type !== 'uplock' && entity.kind === 'segment'
  ...
  try {
    ...
    status(`Error: ${(e as Error).message}`, 'error');
  } finally {
...
async function fetchCountries(): Promise<void> {
  try {
    ...
    log('Failed to load countries: ' + (e as Error).message);
  }
}
...
function saveSettings(): void {
  const val = byId<HTMLInputElement>('wmereq-api-base')?.value.trim().replace(/\/$/, '');
  if (val) {
    apiBase = val;
...
function on(id: string, event: string, fn: EventListener): void { const el = byId(id); if (el) el.addEventListener(event, fn); }

function showStatus(msg: string, type: StatusKind): void {
...
function showFabStatus(msg: string, type: StatusKind): void {
  ...
  fabStatusTimer = setTimeout(() => { el.style.display = 'none'; }, 4000);
}

function clearForm(): void {
  const notes = byId<HTMLTextAreaElement>('wmereq-notes');
...
function debugLog(msg: string): void { if (DEBUG) log(msg); }
