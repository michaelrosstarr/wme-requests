...
(function () {
  ...
  function getVenueEntityFromIssueTracker() {
    if (!getVenueByIdFn) {
      log('getVenueEntityFromIssueTracker: no getVenueByIdFn resolved.');
      return null;
    }
    try {
      const w = pageWindow;
  ...
  function validTypesForKind(kind) {
    return kind
      ? Object.keys(TYPE_ENTITY_KINDS).filter((type) => TYPE_ENTITY_KINDS[type].includes(kind))
      : Object.keys(TYPE_ENTITY_KINDS);
  }
  ...
  async function quickSubmit(type, statusFn = showFabStatus) {
    const entity = getSelectedEntity();
    if (!entity || !entity.items.length) {
      statusFn('Select a segment, map note, or place first.', 'error');
      return;
    }
    ...
    if (type !== 'uplock' && (entity.kind === 'segment' || entity.kind === 'venue')) {
      const lockRankRaw = entity.items.map((it) => it.lockRank).find((r) => r != null);
  ...
  async function doSubmit(type, { countryId, regionId, lockLevel, notes, status }) {
    const entity = getSelectedEntity();
    if (!entity || !entity.items.length) {
      status('Please select a segment, map note, or place first.', 'error');
      return;
    }
    if (!TYPE_ENTITY_KINDS[type].includes(entity.kind)) {
      status(
  ...
  function disableButtons(disabled) {
    [
      'wmereq-btn-submit-downlock',
      'wmereq-btn-submit-uplock',
      'wmereq-btn-submit-imagery',
      'wmereq-btn-submit-accept-pur',
      'wmereq-btn-submit-decline-pur',
      'wmereq-fab-downlock',
      'wmereq-fab-uplock',
      'wmereq-fab-imagery',
      ...QUICK_ACTION_BUTTON_PREFIXES.flatMap((prefix) =>
        QUICK_ACTION_TYPES.map((type) => `${prefix}-${type.replace('_', '-')}`),
      ),
    ].forEach((id) => {
      const btn = byId(id);
      if (btn) btn.disabled = disabled;
    });
    document.querySelectorAll(`.${NATIVE_QUICK_ACTIONS_CLASS} button`).forEach((btn) => {
